...
async function bootstrap() {
  ...
  if (configService.get<string>('NODE_ENV') === 'production') {
    const swaggerUsername =
      configService.get<string>('swagger.username') || 'admin';
    const swaggerPassword =
      configService.get<string>('swagger.password') || 'admin123';
  ...
  logger.log(
    `📚 Swagger documentation available at: http://localhost:${port}/api/v1/api-docs`,
  );
